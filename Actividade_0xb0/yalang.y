...
stm:     	    decl								{$$ = t_stm_new_decl($1);}
        ...
        |	    IF exp THEN LCBRACE stms RCBRACE SEMI				{$$ = t_stm_new_if_else($2,$5,NULL);}
        |	    IF exp THEN LCBRACE stms RCBRACE ELSE LCBRACE stms RCBRACE SEMI	{$$ = t_stm_new_if_else($2,$5,$9);}
        |	    WHILE exp DO LCBRACE stms RCBRACE SEMI				{$$ = t_stm_new_while($2,$5);}
        |           NEXT								{$$ = t_stm_new_next();}
                ;

type:    	    T_INT 			 					{$$ = t_type_new_type(0);}
        |	    T_FLOAT		 	 					{$$ = t_type_new_type(1);}
        |	    T_STRING			 					{$$ = t_type_new_type(2);}
        |	    T_BOOL			 					{$$ = t_type_new_type(3);}
        |	    T_VOID			 					{$$ = t_type_new_type(4);}
        |	    ID				 					{$$ = t_type_new_id($1);}
